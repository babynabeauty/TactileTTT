...
#pi05
sudo -E "$PYTHON_BIN" \
/Users/babyna/TactileTTT/TactileTTT_client_multi.py \
--server-ip 服务器IP \
--server-port 8990 \
...
--server-port 8990 \
--dataset-dir "$DATASET_DIR" \
--policy-input-mode direct16 \
--max-action-chunk-size 16 \
--fps 15 \
--duration 60 \
--prompt "与训练数据完全一致的任务指令"
...
TACTILE_16_HISTORY_OFFSETS = tuple(range(-15, 1))
...
CHUNK_ALIGNED_TACTILE_POLICY_INPUT_MODES = {"direct16", "tactile_ttt"}
...
class WebsocketClientPolicy:
    ...
    def infer(self, obs: Dict) -> Dict:
        ...
        if self._episode_reset_pending:
            # Consumed by openpi.policies.Policy before input transforms. For
            # TactileTTT this clears episode-scoped fast weights; for direct16
            # and baseline policies it is a harmless cache reset.
            request["reset_tactile_memory"] = True
    ...
    def reset(self) -> None:
        # The websocket protocol has no separate reset RPC. Mark the first
        # inference of the new robot run instead, so reset and observation are
        # handled atomically by the server.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "the server-side config chooses calc_force vs raw_force. Use direct16 for "
            "pi05_tactile_direct16 and tactile_ttt for pi05_tactile_ttt_v1."
        ),
    )
    parser.add_argument(
...
def is_chunk_aligned_tactile_mode(policy_input_mode: str) -> bool:
    return policy_input_mode in CHUNK_ALIGNED_TACTILE_POLICY_INPUT_MODES
...
def infer_policy_input_mode(requested_mode: str, metadata: dict | None = None) -> str:
    ...
    if "tactile_ttt" in metadata_text:
        return "tactile_ttt"
    if "direct16" in metadata_text or "tactile_direct16" in metadata_text:
        return "direct16"
    if (
...
def run_policy_once(
    *,
    ...
    fast_executor: concurrent.futures.ThreadPoolExecutor | None,
    state_names: list[str],
    action_names: list[str],
    camera_names: list[str],
    args: argparse.Namespace,
    ...
    client.reset()
    if fast_client is not client:
        fast_client.reset()
    if args.run_mode == "multi":
        print(f"Starting PI0 control loop for run {run_index}. Press RIGHT arrow to finish this run.")
    else:
        print(f"Starting PI0 control loop for run {run_index}...")
    ...
    try:
        with terminal_stop_listener() as stop_requested:
            while time.perf_counter() - start_run < args.duration:
                ...
                # direct16/TTT were trained with one 16-action chunk per tactile
                # memory update. Mid-chunk re-querying changes that cadence and,
                # for TTT, writes the fast weights too often.
                should_query = action_chunk is None or chunk_idx >= n_action_steps
                if not is_chunk_aligned_tactile_mode(policy_input_mode):
                    should_query = should_query or query_count % args.query_frequency == 0
                if should_query:
                    active_async_chunk_id += 1
                    ...
                    try:
                        ...
                        if is_chunk_aligned_tactile_mode(policy_input_mode) and action_chunk.shape[0] != 16:
                            raise ValueError(
                                f"{policy_input_mode} server returned {action_chunk.shape[0]} actions; expected "
                                "exactly 16. Check that the server config matches the checkpoint."
                            )
                        n_action_steps = action_chunk.shape[0]
                ...
                if frame_idx % max(args.fps, 1) == 0:
                    print(f"run={run_index} frame={frame_idx} chunk={chunk_idx}/{n_action_steps} loop={elapsed * 1000:.1f}ms")
                if elapsed < dt:
                    time.sleep(dt - elapsed)
                frame_idx += 1
    finally:
        for future in pending_fast_requests:
            future.cancel()

    suffix = " by RIGHT arrow" if finished_by_key else ""
    print(f"PI0 run {run_index} finished{suffix} after {frame_idx} frames.")
    return frame_idx
...
def main() -> int:
    ...
    if is_chunk_aligned_tactile_mode(preflight_mode) and args.structured_history_offsets == default_history_offsets_arg:
        history_offsets = TACTILE_16_HISTORY_OFFSETS
    elif args.cached_vlm_async_ae and args.structured_history_offsets == default_history_offsets_arg:
    ...
    if (
        is_chunk_aligned_tactile_mode(policy_input_mode)
        and args.structured_history_offsets == default_history_offsets_arg
    ):
        history_offsets = TACTILE_16_HISTORY_OFFSETS
    ...
    if args.cached_vlm_async_ae and is_chunk_aligned_tactile_mode(policy_input_mode):
        raise ValueError("direct16/tactile_ttt must not be combined with --cached-vlm-async-ae.")
    if is_chunk_aligned_tactile_mode(policy_input_mode):
        if history_offsets != TACTILE_16_HISTORY_OFFSETS:
            raise ValueError(
                f"{policy_input_mode} requires 16 consecutive history offsets "
                f"{TACTILE_16_HISTORY_OFFSETS}, got {history_offsets}."
            )
        if args.max_action_chunk_size < 16:
            raise ValueError(
                f"{policy_input_mode} requires --max-action-chunk-size >= 16 so one request executes "
                "the full 16-step training chunk."
            )
    ...
    try:
        ...
        for run_index, total_runs in iter_run_indices(args):
            ...
            run_policy_once(
                ...
                async_ae_offsets=async_ae_offsets,
                run_index=run_index,
            )
