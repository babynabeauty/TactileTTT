...
class Module(nn.Module):
    ...
    def __call__(
        ...
        embedded: Sequence[at.Float[at.Array, "b _t _d"] | None],    #经过词嵌入后的输入 Tensor 列表
        ...
        *,
        ...
        if return_layer_index is not None and return_layer_indices is not None:
            raise ValueError("Specify only one of `return_layer_index` or `return_layer_indices`.")

        if return_layer_indices is not None:
            selected_layers = []
            for requested_index in return_layer_indices:
                layer_index = self.configs[0].depth - 1 if requested_index == -1 else requested_index
                if layer_index < 0 or layer_index >= self.configs[0].depth:
                    raise ValueError(
                        f"return_layer_indices entries must be in [0, {self.configs[0].depth - 1}] or -1, "
